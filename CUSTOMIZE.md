...
You can add news in the about page by adding new Markdown files in a `_news/` directory (not present in this repository; set `announcements.enabled: true` in `_config.yml` to turn the section back on). There are currently two types of news: inline news and news with a link. News with a link take you to a new page while inline news are displayed directly in the about page. The easiest way to create yours is to copy an existing news and modify it.
